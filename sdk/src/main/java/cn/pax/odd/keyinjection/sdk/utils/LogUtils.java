...
public class LogUtils {
    public static final int VERBOSE_LEVEL = Log.VERBOSE;
    public static final int DEBUG_LEVEL = Log.DEBUG;
    public static final int INFO_LEVEL = Log.INFO;
    public static final int WARN_LEVEL = Log.WARN;
    public static final int ERROR_LEVEL = Log.ERROR;
    public static final int ASSERT_LEVEL = Log.ASSERT;
    ...
    private static boolean mDebugSwitchOn = false;  // log 总开关，默认关闭
    private static boolean mDebugSwitchOnLogcat = true;  // logcat 是否打印，默认打印
    private static boolean mDebugSwitchOnLogFile = false; // log 写入文件开关，默认关
    ...
    private static boolean sLogBorderSwitch = false;  // log 边框开关，默认开
    private static int sConsoleFilter = VERBOSE_LEVEL;     // log 控制台过滤器
    private static int sFileFilter = VERBOSE_LEVEL;     // log 文件过滤器
    ...
    public static void file(final String tag, final Object content) {
        log(FILE | DEBUG_LEVEL, tag, content);
    }
    ...
    public static void json(final String content) {
        log(JSON | DEBUG_LEVEL, sGlobalTag, content);
    }
    ...
    public static void json(final String tag, final String content) {
        log(JSON | DEBUG_LEVEL, tag, content);
    }
    ...
    public static void xml(final String content) {
        log(XML | DEBUG_LEVEL, sGlobalTag, content);
    }
    ...
    public static void xml(final String tag, final String content) {
        log(XML | DEBUG_LEVEL, tag, content);
    }
    ...
    private static void log(final int type, final String tag, final Object... contents) {
        if (!mDebugSwitchOn || (!mDebugSwitchOnLogcat && !mDebugSwitchOnLogFile)) {
            return;
        }
        ...
        if (mDebugSwitchOnLogcat && type_low >= sConsoleFilter && type_high != FILE) {
            print2Console(type_low, tagHead.tag, tagHead.consoleHead, body);
        }
//        if ((mDebugSwitchOnLogFile || type_high == FILE) && type_low >= sFileFilter) {
    ...
    private static TagHead processTagAndHead(String tag) {
        if (!sTagIsSpace && !sLogHeadSwitch) {
            ...
            if (sLogHeadSwitch) {
                ...
                String head;
                try (Formatter f1 = new Formatter()
                        .format("%s, %s(%s:%d)",
                                tName,
...
//                    return new TagHead(tag, new String[]{head}, fileHead);
                    return new TagHead(tag, new String[]{fileHead}, fileHead);
                } else {
                    ...
                    String space;
                    try (Formatter f2 = new Formatter().format("%" + spaceLen + "s", "")) {
                        space = f2.toString();
                    }
                    for (int i = 1, len = consoleHead.length; i < len; ++i) {
                        targetElement = stackTrace[i + 3];
                        try (Formatter f3 = new Formatter()
                                .format("%s%s(%s:%d)",
                                        space,
                                        targetElement.getMethodName(),
                                        targetElement.getFileName(),
                                        targetElement.getLineNumber())) {
                            consoleHead[i] = f3.toString();
                        }
                    }
...
//        StringBuilder sb = new StringBuilder();
    ...
    private static synchronized boolean input2File(final String input, final String filePath) {
        if (sExecutor == null) {
    ...
    @IntDef({VERBOSE_LEVEL, DEBUG_LEVEL, INFO_LEVEL, WARN_LEVEL, ERROR_LEVEL, ASSERT_LEVEL})
    ...
    public static class Config {
        ...
        public Config setLogSwitch(final boolean logSwitch) {
            mDebugSwitchOn = logSwitch;
            return this;
        }

        public Config setConsoleSwitch(final boolean consoleSwitch) {
            mDebugSwitchOnLogcat = consoleSwitch;
            return this;
        }
        ...
        public Config setLog2FileSwitch(final boolean log2FileSwitch) {
            mDebugSwitchOnLogFile = log2FileSwitch;
            return this;
        }
        ...
        public String toString() {
            return "switch: " + mDebugSwitchOn
                    + LINE_SEP + "console: " + mDebugSwitchOnLogcat
                    + LINE_SEP + "tag: " + (sTagIsSpace ? "null" : sGlobalTag)
                    + LINE_SEP + "head: " + sLogHeadSwitch
                    + LINE_SEP + "file: " + mDebugSwitchOnLogFile
                    + LINE_SEP + "dir: " + (sDir == null ? sDefaultDir : sDir)
                    + LINE_SEP + "filePrefix" + sFilePrefix
                    + LINE_SEP + "border: " + sLogBorderSwitch
                    + LINE_SEP + "consoleFilter: " + T[sConsoleFilter - VERBOSE_LEVEL]
                    + LINE_SEP + "fileFilter: " + T[sFileFilter - VERBOSE_LEVEL]
